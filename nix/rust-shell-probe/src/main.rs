//! Opt-in layer-shell/SHM/touch feasibility probe. It never launches apps.
//! The layer and event plumbing follows Smithay Client Toolkit's MIT-licensed
//! v0.20.0 simple_layer example, adapted for an idle route socket and touch.
use k230_shell_rust_probe::{
    configure_size, frame_bytes, released_slot, render_probe, Route, TouchTrace,
};
...
const SOCKET_NAME: &str = "k230-shell-rust-probe.sock";
...
impl Probe {
    ...
    fn draw(&mut self, qh: &QueueHandle<Self>) {
        ...
        render_probe(canvas, self.width, self.height, self.route, self.touch);
...
fn main() {
    ...
    let outcome = match args.as_slice() {
        ...
        _ => Err(
            "usage: k230-shell-rust-probe --serve | --surface drawer|shade|settings|hide".into(),
        ),
    };
    if let Err(error) = outcome {
        eprintln!("k230-shell-rust-probe: {error}");
        std::process::exit(1);
    }
}
