...
{ stdenv, pkg-config, wayland-scanner, wayland, wayland-protocols, wlroots_0_20, glib, json-glib, pango, cairo, librsvg }:
stdenv.mkDerivation {
  ...
  buildInputs = [ wayland glib json-glib pango cairo librsvg ];
  buildPhase = ''
    ...
    $CC -std=c11 -O2 -Wall -DK230_CATALOG_LIBRARY -o k230-touch-launcher touch-launcher.c appearance.c catalog.c icon.c \
      wlr-layer-shell-unstable-v1-protocol.c xdg-shell-protocol.c \
      $($PKG_CONFIG --cflags --libs wayland-client gio-unix-2.0 json-glib-1.0 pangocairo librsvg-2.0) -lm -lrt
