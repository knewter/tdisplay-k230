...
static void configure(void *data, struct zwlr_layer_surface_v1 *layer,
	...
	if (!runtime)
		exit(3);
	char path[4096];
	if (snprintf(path, sizeof(path), "%s/drawer-shm-XXXXXX", runtime) >= (int)sizeof(path))
		exit(3);
	int fd = mkstemp(path);
	if (fd < 0)
		exit(3);
	...
	for (size_t i = 0; i < (size_t)width * height; i++)
		pixels[i] = 0xFFFF00FF; /* opaque magenta, unique to this fixture */
	munmap(pixels, bytes);
...
int main(void) {
	...
	zwlr_layer_surface_v1_set_size(layer, 568, 300);
