...
R=pathlib.Path('/run/shell'); S=R/'k230-video.pid'; DEV=pathlib.Path('/dev/video0'); OUT=pathlib.Path('/run/video-recovery-check.json')
U=pwd.getpwnam('shell'); SESSION='/run/current-system/sw/bin/k230-video-session'
...
def ipc(d):
 with socket.socket(socket.AF_UNIX) as s:
  ...
  with s.makefile('rwb',buffering=0) as f:
   f.write(b'{"command":["get_property","time-pos"],"request_id":1}\n')
   ...
   while time.monotonic()<end:
    ...
    if r.get('request_id')==1:
     assert r.get('error')=='success' and isinstance(r.get('data'),(int,float))
     return r['data']
...
report={'provenance':'physical board; production controller; root fault injection; shell UID player','cases':[]}
assert os.geteuid()==0
stop()
     ...
     if b'--vd=h264' in argv:
      ...
      if pos is not None:
       if first is None:first=(time.monotonic(),pos);software_id=d.copy()
        ...
        row['media_times']=[first[1],pos];assert pos>first[1];break
    time.sleep(.1)
