...
An opt-in `k230-foot-session` source wrapper is now packaged for Foot launch
paths. Invoked inside Foot as `k230-foot-session --state-root STATE
...
or keep a terminal open after its app leaves. If `pidfd_open` fails, the
requested app still execs and a fixed diagnostic identifies the missing
follower; no OSC claim is made for that session. A fake PTY with a controlling
...
single 2026-09-23 x86_64 host run observed 14,600 KiB `VmRSS` for one Python
...
python3 tests/test_foot_color_session.py  PASS 6 host tests; one host RSS=14,600 KiB
