...
- `nix build --no-link --print-out-paths --max-jobs 1 --cores 4 .#wifi-settings-broker` — passed for broker source `b75a76fd` before the fresh-interface correction; derivation `/nix/store/f8yg6i50avmfcw8rb0n1kfds381mg4aw-k230-wifi-settings-broker.drv`, package `/nix/store/frqh7xcp937xhfra1kz9j0j77gfby65q-k230-wifi-settings-broker`. A fresh-interface follow-up now changes that source; its exact package build is pending. Earlier package identities are superseded.
