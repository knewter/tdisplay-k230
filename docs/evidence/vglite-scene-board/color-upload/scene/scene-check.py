...
OUT=pathlib.Path('/run/gpu-rgb565-'+('pixman' if FORCE else 'gpu'));OUT.mkdir(exist_ok=True)
PYTHON='/nix/store/v189xydz6qkcd4cbkixcmv91w8hbc560-python3-riscv64-unknown-linux-gnu-3.14.7/bin/python3'
SWAY='/nix/store/wwx2m3hyqhm9jd3ykn6pbik008gm9pbn-sway-unwrapped-riscv64-unknown-linux-gnu-1.12/bin/sway'
...
result={'at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'evidence_class':'board synthetic scene diagnostic; no real finger or optical claim','forced_pixman':FORCE,'source_revision':'d46333f2b838a3440c9ee87ac05dd940cf153284','compositor':SWAY,'client':CLIENT,'boot_id':pathlib.Path('/proc/sys/kernel/random/boot_id').read_text().strip(),'normal_system':os.path.realpath('/run/current-system'),'captures':[]}
def ipc(path,kind=4):
...
proc=subprocess.Popen([PYTHON,'/run/vglite-root-scene-trial.py','--seconds','45','--compositor',SWAY]+(['--force-pixman'] if FORCE else [])+['--',CLIENT,'--app-id','k230.card.one','--duration','30'],stdout=log,stderr=log)
try:
