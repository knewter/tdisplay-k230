...
OUT=pathlib.Path('/run/gpu-cost-'+('pixman' if FORCE else 'gpu')+'-'+ROUND);OUT.mkdir(exist_ok=False)
PYTHON='/nix/store/v189xydz6qkcd4cbkixcmv91w8hbc560-python3-riscv64-unknown-linux-gnu-3.14.7/bin/python3'
SWAY='/nix/store/g3fvr1sf8ix9vfaa48rfsn2cpfnczvi2-sway-unwrapped-riscv64-unknown-linux-gnu-1.12/bin/sway'
...
result={'round':int(ROUND),'profile_enabled':True,'at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'evidence_class':'board synthetic scene diagnostic; no real finger or optical claim','forced_pixman':FORCE,'source_revision':'d92521ecadf53de6eb9b20b25be11d98dbc2bb8a','compositor':SWAY,'client':CLIENT,'boot_id':pathlib.Path('/proc/sys/kernel/random/boot_id').read_text().strip(),'normal_system':os.path.realpath('/run/current-system'),'captures':[]}
def ipc(path,kind=4):
