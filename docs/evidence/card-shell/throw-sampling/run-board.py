#!/usr/bin/env python3
"""Three unchanged injected acceptance runs on the reserved normal board.
...
PACKAGE='/nix/store/mp0bqh0gmq64jfhc2xdishdnd7jlzwln-k230-card-shell'
...
def main():
    ...
    if not re.fullmatch('/var/lib/k230/card-throw-sampling-[a-z0-9-]+',str(args.output)) or args.output.exists():p.error('new protected output required')
    ...
    os.umask(0o077)
    with open('/run/k230-card-throw-sampling.lock','a') as lock:
        ...
        try:
            for index in range(1,4):
                ...
                runtime=Path('/run/k230-card-shell-throw'+str(index))
            ...
            write(args.output/'progress.json',{'state':'completed','finished_runs':3})
