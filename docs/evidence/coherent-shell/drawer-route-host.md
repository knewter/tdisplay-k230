...
The drawer touch model tracks scroll displacement while a finger moves, preserves sampled velocity until release, bounds its offset and flick velocity, stops coasting on a new contact without launching, and cancels tap on a scroll, long press, second contact or Back. A long press opens a cancellable context sheet. Scrolled row artwork and hit regions are clipped to the list viewport. The route listener accepts requests without a blocking read inside the Wayland loop. On an actual app selection, the client unmaps its surface, waits asynchronously with a 1.2-second bound for `card_shell back` to leave the live deck, then launches the chosen app; Cancel simply unmaps the drawer. This is host/source evidence only. The compositor's deck-to-drawer gesture command, Settings and notification data consumers, complete scene synchronization, actual Wayland mapping test, cross-build, installed image and real-finger proof remain open. No OpenSpec task is checked by this partial checkpoint.

Host checks from this source tree:

```text
python3 tests/test_shell_gestures.py
  PASS: drawer-drag, flick-stop, finger-coast, cancel-below-threshold, tap-launch,
