...
LIBRARY='/nix/store/brhzfimak2r3c23lmn80y1g6ww6nmb1r-pixman-riscv64-unknown-linux-gnu-0.46.4/lib/libpixman-1.so'
...
def run_case_set(binary, target, prefix, vector, corrupt=False):
    target.mkdir()
    env=os.environ.copy()
...
def measure(args):
    binary=args.package/'bin/k230-pixman-rvv-pixel-probe'
    result={'status':'FAIL','evidence_class':'physical-board-pixman-pixels' if args.probe else 'qemu-user-pixman-pixels',
            'observed_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
            'package':str(args.package),'probe_sha256':digest(binary),'library':LIBRARY,'library_sha256':digest(LIBRARY),
            'checker_sha256':digest(__file__),'token':args.token,
            'limits':['Instrumented callbacks prove dispatch and pixels, not renderer cost.','Declared integer RGB565/ARGB/XRGB cases only; no exhaustive Pixman or physical display claim.']}
    if args.probe:
        ...
        if model!='LILYGO T-Display-K230':raise RuntimeError('wrong physical board')
        ...
        result.update(current_system=SYSTEM,model=model,boot_id=Path('/proc/sys/kernel/random/boot_id').read_text().strip(),context=context)
        prefix=[]
    else:
    ...
    with tempfile.TemporaryDirectory(prefix='k230-pixman-pixels-',dir='/run' if args.probe else None) as tmp:
        root=Path(tmp)
        scalar=run_case_set(binary,root/'scalar',prefix,False)
        vector=run_case_set(binary,root/'vector',prefix,True)
        corrupt=run_case_set(binary,root/'corrupt',prefix,True,True)
...
def main():
    ...
    args=p.parse_args()
    if not re.fullmatch(r'/nix/store/[a-z0-9]{32}-k230-pixman-rvv-pixel-probe-[A-Za-z0-9._+-]+',str(args.package)):p.error('expected immutable pixel-probe package')
    if args.probe:
        if not args.token or not re.fullmatch('[a-f0-9]{32}',args.token):p.error('requires capture token')
        report=measure(args);print('K230_PIXELS_RESULT '+json.dumps(report,sort_keys=True),flush=True)
    else:
        ...
        if args.board:
            ...
            cmd=shlex.join([PYTHON,'-I',remote,'--probe','--token',args.token,'--package',str(args.package)])
            ...
            if report['token']!=args.token:raise RuntimeError('stale pixel report')
            (args.output/'report.serial.log').write_text(lines[0]+'\n')
        else:report=measure(args)
