...
written only on success.** `k230_display_logo()` sets an environment
variable when the image loaded and the init sequence was sent;
`ft_board_setup()` — which the vendor already uses to rewrite the memory
node — copies it into `/chosen` as a boolean. The kernel reads
`of_chosen`. A boot on which the logo file is missing, or the load fails,
has no flag and the kernel lights the panel itself. Rejected: LILYGO's
static `canaan,preserve-boot-splash` in the panel node (patch 0051). It is
simpler, and it turns every stage 1 hiccup into a permanently dark screen
with no log line to say why. Rejected: passing it on the kernel command
line — `bootargs.txt` is generated by Nix before stage 1 runs and cannot
know what stage 1 will manage to do. Layer: **stage 1 → device tree →
kernel**.
