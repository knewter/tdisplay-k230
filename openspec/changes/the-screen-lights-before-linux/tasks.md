...
- [x] 3.4 Put `logo.xrgb` on the boot partition from `nix/sd-image.nix`, behind the option from 6.1, and add the splash file to the list of things stage 1 requires of the partition in `docs/evidence/uboot-env.txt`. Verify with `nix build .#sdImage` and `debugfs -R 'ls /' <boot partition>` listing `logo.xrgb` beside `Image` — laptop claim
...
prescribed second-card procedure has not been performed. The splash-enabled
diagnostic full-image build and host-side boot-partition
inspection now satisfy 3.4; see `docs/evidence/splash-preserve-trial/build.txt`
and `image-contents.txt` in that directory. This does not close a physical
handoff task.
...
- [ ] 4.3 **Hardware.** Boot the new kernel on the card from 3.6 and leave the board at the serial prompt with nothing opening the display. Verify by committing a capture to `docs/evidence/boot-splash-handoff.md` showing `/proc/device-tree/chosen` carrying the flag, the driver's "stage 1 splash: leaving fbdev unset" message, no panel prepare/init sequence before a DRM client starts, `ls /dev/fb0` failing, and a photograph of the splash still on the glass at the prompt. The panel driver's "left as stage 1 set it" line belongs to the first DRM modeset in 4.4, not the idle-kernel phase — hardware claim
- [x] 4.4 **Hardware.** Film the first modeset: with both automatic display services masked and the webcam recording, start the unchanged immutable `k230-drm-splash` under a temporary `first-modeset-owner.service` copied from the built unit. Its first `drmModeSetCrtc` must scan out the same stage-1 logo converted to RG16; record its format/connector/CRTC/framebuffer line. Stock `modetest` cannot load the logo (its `-F` selects built-in patterns), so it cannot perform this same-image check. AR24 is optional only if the selected primary plane advertises it. Verify by committing the frame-by-frame result to `docs/evidence/boot-splash-handoff.md`: record whether the first modeset's VO/DSI initialization and commit path produces a dark interval, its measured length, and whether the panel recovers without replaying its init sequence. **This task decides the fallback**: if it shows a dark interval, restore reset + init in prepare, re-run, record the blink, and restate the boot-splash requirement to what was measured — hardware claim
- [x] 4.5 **Hardware.** Boot the same kernel on a card with `logo.xrgb` removed. Verify by committing a capture showing the flag absent from `/proc/device-tree/chosen`, the panel driver sending the init sequence, `/dev/fb0` present, and a photograph of the console on the panel — this is the "stage 1 did not light it" scenario and it must still look like `panel-lit.md`. Release the board — hardware claim
...
2026-09-22 controlled first-modeset acceptance: `docs/evidence/splash-first-modeset-preserve/README.md` and its frame-by-frame audit close 4.4: 420 consecutive frames around owner startup retain the logo; the later shell transition also has no captured dark frame. The matching kernel journal records first VO/DSI preservation and the prepare skip. Task 4.3 retains its unmet second-card prerequisite; 5.4 retains its power-on requirement. Normal automatic startup was restored.

2026-09-22 option checks: `docs/evidence/splash-option-acceptance/README.md` records both toplevel derivation evaluations and a fresh canonical console-image build/boot-partition inspection. These host checks pass; 6.1 remains open because the final effective splash default still awaits physical acceptance.
