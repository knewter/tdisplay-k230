...
- [ ] 4.1 In the U-Boot overlay, keep process-local readiness false until `k230_display_logo()` has loaded the image and sent the init sequence; `ft_board_setup()` writes that state into `/chosen` as a boolean without using the persistent environment. Verify with `nix build .#uboot-k230` and `strings result/u-boot | grep` finding the property name — laptop claim
