...
- [ ] 2.5 Remeasure the enabled closure after adding the touch-menu monitor utility (`htop`). Verify with `nix build .#nixosConfigurations.k230.config.system.build.toplevel` and append the wall-clock time, derivation count, and closure size to `docs/evidence/shell-build.txt`. This is a build-host claim only; do not run it concurrently with another full closure/image build.
