...
Whether this board can be recovered over USB with no bootloader on the card at
all SHALL be established by observation and written down — including which
button forces it, what the host sees, and which tool drives it — or recorded
as not working. It SHALL NOT be left as an untested belief that a later change
relies on.

*Grounding: `docs/uboot-ums.md` §5. Canaan's on-target half is readable in
`buildroot-overlay/boot/uboot/u-boot-2022.10-overlay/board/canaan/common/k230_board_common.c`:
`do_k230_dfu()` decodes a burn configuration from SRAM, sets `dfu_alt_info` to
`mmc 1=…` for `KBURN_USB_ISP_SDIO1`, and runs `dfu 0`. The host half is
`kendryte/k230_flash_py` (MIT), which enumerates the device as VID `0x29f1`
PID `0x0230` — the same pair `k230_canmv_burntool_defconfig` sets as
`CONFIG_USB_GADGET_VENDOR_NUM` / `CONFIG_USB_GADGET_PRODUCT_NUM`. The
schematic puts a pushbutton `SW3` between `BOOT0` and ground.*

<!-- UNVERIFIED: the fall-through from a failed boot medium to USB boot has
not been observed on this board. Grounded once
docs/evidence/uboot-ums-hardware.txt records what the host enumerates with the
card removed. -->

#### Scenario: A person is asked how bad it would be to brick the bootloader

- **WHEN** the question is asked
- **THEN** the answer names how many independent ways the board can be recovered, and each one has been tried rather than inferred
