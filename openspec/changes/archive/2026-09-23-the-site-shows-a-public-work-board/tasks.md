The named work-board generator and test commands are implemented host checks. Publication is recorded separately below.
...
- [x] 3.3 After the detail view lands, inspect the published `/work/` URL at its new revision, directly open an item and verify its local documents/links and close behavior; record URL, CI run, UTC observation and limits in `docs/evidence/spec-site-work-board/README.md`, then run `python3 scripts/build_site.py`.
- [x] 3.4 After the built and published detail view meets every task, run `openspec validate --all --strict`, sync the delta and archive the change. Keep it open if publication or evidence remains unverified; inspect the archive push's CI and Pages revision as handoff follow-through.
