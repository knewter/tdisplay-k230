...
Host policy, narrow package and evaluated service proof: `docs/evidence/storage-capacity/implementation.md`. The actual disposable RISC-V guest passes all seven filesystem/preservation cases in `docs/evidence/storage-capacity/qemu/result.json`. These complete tasks 1.1, 1.2 and 2.1. `docs/evidence/storage-capacity/image-build.json` records the matching system/image build and actual compact boot-partition inspection for 2.2. Physical first/repeat boots now pass in `board-before.json`, `board-after.json` and `board-repeat.json` beside that evidence. The image system closure and exact boot files were installed on the existing compact root, without a full reflash or manual resize. Publication of the completed physical evidence and handheld update passed build/deploy run 35921034930 for master `a20c0492`; `publication.json` records the actual published-page response. All eight tasks are complete.
