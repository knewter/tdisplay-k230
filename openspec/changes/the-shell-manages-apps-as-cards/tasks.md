...
- [ ] 5.1 Build the selected component and integrate it into a system image only after host tests pass; verify the system closure with `nix build .#nixosConfigurations.k230.config.system.build.toplevel` and separately record a QEMU lifecycle smoke run with the proposed-to-create `tools/qemu-k230.sh --card-shell-smoke`. QEMU does not prove panel, touch, or live presentation.
...
- [ ] 5.3 On the physical board, capture a focused real-finger trial of shrink, horizontal deck drag, expand, upward throw, and recovery; verify with the proposed-to-create `python3 tools/capture-feature.py --feature card-shell --provenance real-touch --duration 30` plus a committed audit that distinguishes camera visibility from native state evidence.
