...
- [ ] 2.1 Write `display-rm69a10-568x1232.dtsi` for `panel-canaan-universal`, modelled on `display-st7701-480x800.dtsi`, carrying the sequence from 1.1. Verify by building the DTB with `nix build .#deviceTree` and checking it compiles without warnings
